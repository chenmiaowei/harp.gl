...
export class MapViewAtmosphere {
    ...
    constructor(
        ...
        this.addToScene(this.m_mapView.scene);
    ...
    set enabled(enable: boolean) {
        ...
        if (enable && !isAdded) {
            ...
            this.removeFromScene(this.m_mapView.scene);
    ...
    set lightMode(lightMode: AtmosphereLightMode) {
        ...
        if (this.m_skyMaterial !== undefined) {
            const skyMat = this.m_skyMaterial as GroundAtmosphereMaterial;
     ...
     */
    private addToScene(scene: THREE.Scene) {
        assert(!MapViewAtmosphere.isPresent(scene), "Atmosphere already added");
        if (this.m_skyMesh !== undefined) {
            scene.add(this.m_skyMesh);
        }
        if (this.m_groundMesh !== undefined) {
            scene.add(this.m_groundMesh);
        }
    }
     ...
     */
    private removeFromScene(scene: THREE.Scene) {
        if (MapViewAtmosphere.isPresent(scene)) {
            if (this.m_skyMesh !== undefined) {
                scene.remove(this.m_skyMesh);
            }
            if (this.m_groundMesh !== undefined) {
                scene.remove(this.m_groundMesh);
            }
        }
    }
    ...
    private createGroundGeometry() {
        ...
        if (this.m_materialVariant === AtmosphereShadingVariant.ScatteringShader) {
            ...
            this.m_groundMaterial = new THREE.MeshBasicMaterial({
                color: new THREE.Color(0x11899a),
                opacity: 0.4,
                transparent: true,
                visible: true,
                depthTest: true,
    ...
    private setupSkyForRendering(): void {
        ...
        this.m_skyMesh.onBeforeRender = (
            ...
            assert(_material === this.m_skyMaterial);
    ...
    private setupGroundForRendering(): void {
        ...
        this.m_groundMesh.onBeforeRender = (
            ...
            assert(_material === this.m_groundMaterial);
