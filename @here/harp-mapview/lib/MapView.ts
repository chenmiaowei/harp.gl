...
export class MapView extends THREE.EventDispatcher {
     ...
     */
    get tilt(): number {
        const { pitch } = MapViewUtils.extractAttitude(this, this.m_camera);
     ...
     */
    get heading(): number {
        const { yaw } = MapViewUtils.extractAttitude(this, this.m_camera);
    ...
    private lookAtImpl(params: LookAtParams): void {
        // let zoomLevel = MapViewUtils.roundZoomLevel(zoomLevel);
        ...
        const tilt = Math.min(getOptionValue(params.tilt, this.tilt), PITCH_LIMIT_DEG);
